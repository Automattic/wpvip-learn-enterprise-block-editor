...
function modifyBlockTransforms(settings, name) {
  ...
  if (name === 'core/heading') {
    settings.transforms = {
      ...settings.transforms,
      from: settings.transforms?.from?.filter(transform => !(transform.type === 'block' && transform.blocks?.includes('core/paragraph'))) || []
    };
  }
  ...
  if (name === 'core/paragraph') {
    settings.transforms = {
      ...
      to: [...(settings.transforms?.to || []), {
        ...
        isMatch: attributes => attributes.className?.includes('custom-class')
      }]
    };
  }
  return settings;
}
(0,_wordpress_hooks__WEBPACK_IMPORTED_MODULE_0__.addFilter)('blocks.registerBlockType', 'my-plugin/modify-paragraph-transforms', modifyBlockTransforms);
})();
